...
class EightPuzzleSolver {
    ...
    int row[4] = {1, 0, -1, 0};
    int col[4] = {0, -1, 0, 1};
    string dir[4] = {"Down", "Left", "Up", "Right"};

public:
    EightPuzzleSolver(vector<vector<int>> ini, vector<vector<int>> fin)
        : initial(move(ini)), goal(move(fin)) {}
    ...
    void solve() {
        ...
        Node start = {initial, sx, sy, manhattan(initial), 0, ""};
        pq.push(start);

        while (!pq.empty()) {
            Node current = pq.top(); pq.pop();

            cout << "Level: " << current.level << ", Heuristic (h): " << current.cost
                 << ", f(n) = g + h = " << current.level + current.cost << "\n";
...
private:
    int manhattan(const vector<vector<int>> &curr) {
        int dist = 0;
        for(int i = 0; i < 3; i++)
            for(int j = 0; j < 3; j++) {
...
int main() {
    ...
    cout << "Enter Initial Matrix (0 as blank): ";
    for (int i = 0; i < 9; ++i)
        cin >> ini[i / 3][i % 3];

    cout << "Enter Goal Matrix (0 as blank): ";
    for (int i = 0; i < 9; ++i)
        cin >> fin[i / 3][i % 3];

    EightPuzzleSolver solver(ini, fin);
    solver.solve();
